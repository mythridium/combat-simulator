...
export abstract class SimClasses {
    ...
    private static async simManager() {
        const { SimManager } = await import(/* webpackMode: "eager" */ 'src/worker/simulator/sim-manager');

        return SimManager;
    }

    private static async simPlayer() {
        const { SimPlayer } = await import(/* webpackMode: "eager" */ 'src/worker/simulator/sim-player');

        return SimPlayer;
    }

    private static async simEnemy() {
        const { SimEnemy } = await import(/* webpackMode: "eager" */ 'src/worker/simulator/sim-enemy');
