...
export class SimPlayer extends Player {
    ...
    constructor(simManager: SimManager, simGame: SimGame) {
        ...
        this.dataNames = {
            ...
            strings: ['currentGamemodeID', 'pillarID', 'pillarEliteID']
        };
    }
    ...
    initialize() {
        ...
        this.equipmentSets.forEach(e => {
            e.equipment.removeQuantityFromSlot = (slot: SlotTypes, quantity: number): boolean => {
                switch (slot) {
                    case 'Summon1':
                    case 'Summon2':
    ...
    addAgilityModifiers() {
        ...
        for (let obstacleIndex = 0; obstacleIndex < this.course.length; obstacleIndex++) {
        ...
        if (fullCourseForPillar && this.pillarID) {
            this.modifiers.addModifiers(
                this.micsr.actualGame.agility.pillars.allObjects.find(p => p.id === this.pillarID)!.modifiers
            );
        }
        if (fullCourse && this.pillarEliteID) {
            this.modifiers.addModifiers(
                this.micsr.actualGame.agility.elitePillars.allObjects.find(p => p.id === this.pillarEliteID)!.modifiers
            );
        }
    }
    ...
    encode(writer: SaveWriter) {
        ...
        this.dataNames.booleanArrays.forEach((x: PropertyKey) => {
            if (this.hasKey(this, x)) {
                this.micsr.logger.verbose('encode boolean array', x, this[x]);
        ...
        this.dataNames.numberArrays.forEach((x: PropertyKey) => {
            if (this.hasKey(this, x)) {
                this.micsr.logger.verbose('encode number array', x, this[x]);
        ...
        this.dataNames.booleans.forEach((x: PropertyKey) => {
            if (this.hasKey(this, x)) {
                this.micsr.logger.verbose('encode boolean', x, this[x]);
        ...
        this.dataNames.numbers.forEach((x: PropertyKey) => {
            if (this.hasKey(this, x)) {
                this.micsr.logger.verbose('encode number', x, this[x]);
        ...
        this.dataNames.strings.forEach((x: PropertyKey) => {
            if (this.hasKey(this, x)) {
                this.micsr.logger.verbose('encode string', x, this[x]);
        ...
        this.micsr.logger.verbose('encode skillLevel', this.skillLevel);
        //writer.writeString(this.potion?.id || "");
        this.micsr.logger.verbose('encode potion id', this.potion?.id);
        writer.writeArray(this.petUnlocked, (p, w) => w.writeNamespacedObject(p));
        this.micsr.logger.verbose('encode petUnlocked', this.petUnlocked);
        return writer;
    }
    ...
    decode(reader: SaveWriter, version: number) {
        ...
        this.dataNames.booleanArrays.forEach((x: PropertyKey) => {
            if (this.hasKey(this, x)) {
                ...
                this.micsr.logger.verbose('decode boolean array', x, this[x]);
            } else {
                throw new Error(`Missing key: ${x.toString()}`);
            }
        });
        this.dataNames.numberArrays.forEach((x: PropertyKey) => {
            if (this.hasKey(this, x)) {
                ...
                this.micsr.logger.verbose('decode number array', x, this[x]);
            } else {
                throw new Error(`Missing key: ${x.toString()}`);
            }
        });
        this.dataNames.booleans.forEach((x: PropertyKey) => {
            if (this.hasKey(this, x)) {
                ...
                this.micsr.logger.verbose('decode boolean', x, this[x]);
            } else {
                throw new Error(`Missing key: ${x.toString()}`);
            }
        });
        this.dataNames.numbers.forEach((x: PropertyKey) => {
            if (this.hasKey(this, x)) {
                ...
                this.micsr.logger.verbose('decode number', x, this[x]);
            } else {
                throw new Error(`Missing key: ${x.toString()}`);
            }
        });
        this.dataNames.strings.forEach((x: PropertyKey) => {
            if (this.hasKey(this, x)) {
                ...
                this.micsr.logger.verbose('decode string', x, this[x]);
            } else {
                throw new Error(`Missing key: ${x.toString()}`);
            }
        });
        ...
        this.micsr.logger.verbose('decode skillLevel', this.skillLevel);
