...
export class AgilityCourse {
    ...
    constructor(parent: App, player: SimPlayer, filters: IAgilityFilter[]) {
        ...
        this.agilityObstacles = [
            noObstacle,
            ...this.parent.game.agility.actions.allObjects.map((x, index) => {
                ...
                this.filters.forEach(
                    filter =>
                        (obstacle[filter.tag] =
                            this.micsr.showModifiersInstance.printRelevantModifiers(x.modifiers, filter.tag).length > 0)
                );
                return obstacle;
            })
        ];
        this.agilityPillars = [
            noObstacle,
            ...this.parent.game.agility.pillars.allObjects.map((x, index) => {
                const pillar: IObstacle = {
                    category: -1,
                    description: '',
                    index: index,
                    id: x.id,
                    media: [this.media.combat, this.media.statistics, this.media.gp, this.media.stamina][index],
                    modifiers: x.modifiers,
                    name: x.name,
                    requirements: {}
                };
        ...
        this.eliteAgilityPillars = [
            noObstacle,
            ...this.parent.game.agility.elitePillars.allObjects.map((x, index) => {
                const pillar: IObstacle = {
                    category: -1,
                    description: '',
                    index: index,
    ...
    selectBlueprint(event: Event) {
        ...
        this.parent.actualGame.agility.actions.allObjects.forEach((obstacle, i) => {
    ...
    createBlueprints() {
        ...
        this.blueprintDropdown = this.parent.agilitySelectCard.addDropdown('Blueprints', text, values, (event: any) =>
            this.selectBlueprint(event)
        ).container;
    ...
    addObstacleMultiButton(
        ...
        if (category === 'elite') {
            ...
            menuItems = this.agilityObstacles.filter(
                x => x.category === -1 || (x[filter] === isProp && x.category === category)
            );
        }
    ...
    getObstacleTooltip(category: number | 'pillar' | 'elite', obstacle: IObstacle) {
        ...
        this.tmpModifiers.addModifiers(obstacle.modifiers, negativeMultiplier);
    ...
    createAgilityPopup(category: number | 'pillar' | 'elite', filter: IAgilityFilter) {
        ...
        if (category === 'pillar') {
            ...
            this.addObstacleMultiButton(obstacleSelectCard, 'Elite Pillars', category, filter.tag, true);
        } else {
    ...
    selectObstacle(category: number | 'pillar' | 'elite', obstacle: IObstacle) {
        ...
        if (category === 'pillar') {
            ...
            const existingObstacle = this.parent.game.agility.actions.allObjects.find(
                (_, i) => i === this.player.course[category]
            )!;
     ...
     */
    setObstacleImage(category: number | 'pillar' | 'elite', obstacle: IObstacle) {
    ...
    importAgilityCourse(courseObstacles: number[], masteries: boolean[], pillarID: string, elitePillarID: string) {
        ...
        if (pillarID !== undefined) {
            this.selectObstacle(
                'pillar',
                this.agilityPillars[this.player.game.agility.pillars.allObjects.findIndex(v => v.id === pillarID) + 1]
            );
        }
        ...
        if (elitePillarID !== undefined) {
            this.selectObstacle(
                'elite',
                this.eliteAgilityPillars[
                    this.player.game.agility.elitePillars.allObjects.findIndex(v => v.id === elitePillarID) + 1
                ]
            );
        }
        // set image selection
