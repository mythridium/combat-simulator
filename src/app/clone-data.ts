...
export interface ICloneDictionary {
    [name: string]: string;
}
...
export class CloneData {
    ...
    modifierData = () => {
        ...
        const knownKeys = ['description', 'isNegative', 'isSkill', 'langDescription', 'tags'];
        for (const key in base) {
            // @ts-expect-error HACK
            const data = { ...base[key] };
    ...
    restoreModifierData = () => {
        const clone: ICloneDictionary = {};
        const base = modifierData;
        const knownKeys = ['description', 'isNegative', 'isSkill', 'langDescription', 'tags', 'modifyValueClone'];
        for (const key in base) {
            // @ts-expect-error HACK
            const data = base[key];
            for (const propKey in data) {
