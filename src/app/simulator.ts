...
import { SimEnemy } from './sim-enemy';
import { CustomEventMatcher, SimGame } from './sim-game';
import { ISimResult, ISimStats, SimManager } from './sim-manager';
...
export class Simulator {
    ...
    async createWorker(): Promise<Worker> {
        return new Worker(this.workerURL, { name: 'Worker' });
    }
    ...
    sendScripts(worker: Worker, i: number) {
        return new Promise<void>(resolve => {
            ...
            worker.onerror = (event: any) => {
                this.micsr.logger.log('An error occurred in a simulation worker');
    ...
    intializeWorker(worker: any, i: any) {
        ...
        constantNames.forEach(constant => (constants[constant.name] = constant.data));
        // functions
        const functionNames: {
            name: string;
            data: any;
        ...
        [
            ...
            if (window[func] === undefined) {
                this.micsr.logger.error(`window[${func}] is undefined`);
            }
    ...
    processWorkerMessage(event: any, workerID: any, resolve: (result?: any) => void) {
        ...
        if (event.data.simResult?.simSuccess === false) {
            this.micsr.logger.log({ ...event.data.simResult });
        }
        switch (event.data.action) {
            case 'SCRIPTS_DONE':
                if (event.data.error) {
                    this.micsr.logger.error(event.data.error);
                }
                resolve();
                break;
            case 'FINISHED_SIM':
