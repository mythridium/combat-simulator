...
export class ShowModifiers {
    ...
    constructor(micsr: MICSR, name: any, logName: any, check = true) {
        ...
        this.creasedModifiers = {
            ...
            attack: [],
            strength: [],
            melee: ['MeleeAccuracyBonus', 'MeleeStrengthBonus', 'MeleeMaxHit', 'MeleeLifesteal', 'MeleeCritChance'],
            ...
            prayer: ['ChanceToPreservePrayerPoints', 'FlatPrayerCostReduction', 'PrayerCost'],
            ...
            production: ['GlobalPreservationChance', 'SkillPreservationChance'],
            mastery: ['GlobalMasteryXP', 'MasteryXP'],
            // specific skills
            agility: ['GPFromAgility', 'AgilityObstacleCost'],
            altMagic: ['AltMagicSkillXP', 'AltMagicRunePreservation'],
            astrology: [],
            cooking: [
            ...
            crafting: [],
            farming: ['ChanceDoubleHarvest', 'FarmingYield', 'AllotmentSeedCost'],
            firemaking: ['ChanceForDiamondFiremaking', 'GPFromFiremaking'],
            fishing: ['FishingSpecialChance'],
            fletching: [],
            herblore: ['ChanceRandomPotionHerblore'],
            mining: ['ChanceNoDamageMining', 'ChanceToDoubleOres', 'MiningNodeHP'],
            runecrafting: ['ChanceForElementalRune', 'ElementalRuneGain', 'AdditionalRunecraftCountRunes'],
            smithing: ['SeeingGoldChance'],
            summoning: ['SummoningMaxHit', 'SummoningAttackIntervalPercent'],
            nonCBSummoning: ['SummoningShardCost', 'SummoningCreationCharges'],
            thieving: [
            ...
            cartography: [],
            archaeology: [],
            woodcutting: ['BirdNestDropRate'],
            // golbin raid
            golbinRaid: [],
            // aprilFools
            aprilFools: [],
            // modifiers that are not actually implemented in the game
            unimplemented: ['MaxStamina', 'StaminaCost', 'StaminaPerObstacle', 'StaminaPreservationChance']
        };

        // unique modifiers, i.e. not in+de creased
        this.singletonModifiers = {
            misc: ['autoSlayerUnlocked', 'dungeonEquipmentSwapping', 'increasedEquipmentSets'],
            skilling: ['allowSignetDrops', 'increasedMasteryPoolProgress'],
            combat: [
                'meleeProtection',
                'rangedProtection',
                'magicProtection',
            ...
            hitpoints: ['decreasedRegenerationInterval'],
            defence: [],
            attack: [],
            strength: [],
            melee: [
            ...
            ranged: ['summoningSynergy_7_8', 'summoningSynergy_7_12', 'summoningSynergy_7_14', 'summoningSynergy_7_15'],
            magic: [
                'increasedConfusion',
                'increasedDecay',
            ...
            slayer: ['summoningSynergy_2_12', 'summoningSynergy_8_12'],
            prayer: ['increasedRedemptionPercent', 'increasedRedemptionThreshold'],
            nonCombat: ['increasedOffItemChance', 'doubleItemsSkill'],
            production: [],
            mastery: [],
            // specific skills
            agility: [],
            altMagic: [],
            astrology: ['increasedBaseStardustDropQty'],
            cooking: [
            ...
            farming: ['freeCompost', 'increasedCompostPreservationChance'],
            firemaking: [
                'freeBonfires',
            ...
            fishing: ['summoningSynergy_3_5', 'summoningSynergy_4_5', 'summoningSynergy_5_9', 'summoningSynergy_5_18'],
            fletching: [],
            herblore: [],
            mining: [
        ...
        // slayer
        this.relevantModifiers.slayer = this.getModifierNames(['skilling'], [this.micsr.skillIDs.Slayer]);
        ...
        this.productionSkills.forEach((name: any) => {
            const setNames = ['skilling', 'nonCombat', 'production', 'mastery'];
            if (name === 'Summoning') {
                setNames.push('nonCBSummoning');
            }
            this.relevantModifiers[name] = this.getModifierNames(setNames, [this.micsr.skillIDs[name]]);
        });

        // whatever alt magic is
        this.relevantModifiers.altMagic = this.getModifierNames(['skilling', 'nonCombat', 'altMagic'], []);

        // golbin raid
        this.relevantModifiers.golbinRaid = this.getModifierNames(['golbinRaid'], []);
    }
    ...
    printDiffModifier(modifier: any, value: any, skillID: string = undefined) {
        // compute difference
        if (!value) {
            return [];
        }
    ...
    getModifierValue(modifiers: any, modifier: any, skillID: string = undefined) {
    ...
    showRelevantModifiers(modifiers: any, text: any, tag = 'all') {
        ...
        passives += '<br/>';
        this.gatheringSkills.forEach(
            (skill: any) =>
                (passives += this.makeTagButton(
                    skill,
                    skill,
                    `assets/media/skills/${skill.toLowerCase()}/${skill.toLowerCase()}.svg`
                ))
        );
        passives += '<br/>';
        this.productionSkills.forEach(
            (skill: any) =>
                (passives += this.makeTagButton(
                    skill,
                    skill,
